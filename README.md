...
项目现已提供一套独立的商家后台入口，便于演示“普通用户”和“商家用户”的体验差异：

- 登录 `merchant` 账号后会直接进入 `/merchant`
- 商家可在后台查看店铺资料、商品总数、在售商品数、订单总数、待处理订单数和最近订单
- `/merchant/products` 支持新增商品、分页查看、编辑价格/库存等字段，以及删除前确认
- `/merchant/orders` 只展示与当前店铺商品相关的订单项
- `admin` 可通过 `merchant_id` 参数查看指定店铺的后台数据

建议演示路径：

```text
商家登录 -> 商家后台总览 -> 新增商品 -> 修改库存/价格 -> 查看相关订单
```

当前实现默认把“当前商家”解释为该账号最早创建的店铺；这样可以在保留历史多店铺数据兼容性的同时，给演示场景一个稳定、直观的入口。
