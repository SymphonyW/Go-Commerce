...
| 订单 | 创建、查询、取消、发货、完成、状态机、超时关闭 | 创建订单已实现落库幂等记录。 |
| 支付 | 创建模拟支付、查询、标记成功、标记失败 | 支付成功通过事件异步推动订单变为 `paid`。 |
...
> 当前前端请求封装尚未自动生成并携带 `Idempotency-Key`，因此完整下单 / 支付主链路建议优先使用下方 curl 或测试用例验证。
...
- 为取消订单、支付成功补齐真正落库的幂等重放，而不仅是状态天然幂等。
