...
func TestBusinessMetricsRecordDomainSignals(t *testing.T) {
	...
	assertCounterValue(t, registry, "go_commerce_order_created_total", map[string]string{"service": "order-service", "result": "success"}, 1)
	assertCounterValue(t, registry, "go_commerce_order_created_total", map[string]string{"service": "order-service", "result": "failure"}, 1)
	assertCounterValue(t, registry, "go_commerce_inventory_insufficient_total", map[string]string{"service": "order-service"}, 1)
